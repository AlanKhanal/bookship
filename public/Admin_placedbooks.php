...
<?php
$filter=mysqli_query($conn,"SELECT uniqueorder,mail,phno,customer,`state`,city,street,hono,postC,method,orderedTime FROM orders WHERE orderStatus=0 GROUP BY uniqueorder");
$fil="null";//fil is used to confirm filtered orders
if(isset($_REQUEST['latest'])){
    $filter=mysqli_query($conn,"SELECT uniqueorder,mail,phno,customer,`state`,city,street,hono,postC,method,orderedTime FROM orders WHERE orderStatus=0 GROUP BY uniqueorder ORDER BY orderedTime desc");
    $fil="latest";
}
if(isset($_REQUEST['oldest'])){
    $filter=mysqli_query($conn,"SELECT uniqueorder,mail,phno,customer,`state`,city,street,hono,postC,method,orderedTime FROM orders WHERE orderStatus=0 GROUP BY uniqueorder ORDER BY orderedTime asc");
    $fil="oldest";
}
if(isset($_REQUEST['esewa'])){
    $filter=mysqli_query($conn,"SELECT uniqueorder,mail,phno,customer,`state`,city,street,hono,postC,method,orderedTime FROM orders WHERE orderStatus=0 and method='esewa' GROUP BY uniqueorder");
    $fil="esewa";
}
if(isset($_REQUEST['cash'])){
    $filter=mysqli_query($conn,"SELECT uniqueorder,mail,phno,customer,`state`,city,street,hono,postC,method,orderedTime FROM orders WHERE orderStatus=0 and method='cash' GROUP BY uniqueorder");
    $fil="cash";
}
...
if(isset($_REQUEST['search'])){
    $srch=$_REQUEST['srch'];
    $filter=mysqli_query($conn,"SELECT uniqueorder,mail,phno,customer,`state`,city,street,hono,postC,method,orderedTime FROM orders WHERE orderStatus=0 and uniqueorder LIKE '%$srch%' GROUP BY uniqueorder");
    $fil="search";
...
<?php
    ...
    <tr>
        ...
        ?>
                ...
                <div class="choice" >
                    <a href="shipmentdecision.php?oID=<?=$uID?>&des=1" class="confirm" style="">CONFIRM</a>
                    <a href="shipmentdecision.php?oID=<?=$uID?>&des=cancel" class="cancel">CANCEL</a>
                </div>
                </th>
            </tr>
    <!-- ?> -->
    ...
    echo "<div style='margin:1rem 2rem;'>NO ORDER FOUND.";
}
?>        </div>
    </div>
</body>
